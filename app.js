(() => {
  ...
  function openDriveFolderWeb(id /* 不再使用 preWin */) {
    ...
    // === Android：沿用原本手機邏輯（直接走 intent），打不開就退回 web（同窗）===
    if (isAndroid) {
      try {
    ...
    // === 桌機：套用同樣邏輯（先試 App，失敗退 web），不再開新分頁 ===
    try {
      // 用隱形 iframe 嘗試喚起 App，不會留下錯誤頁或空白頁
      const ifr = document.createElement("iframe");
      ifr.style.display = "none";
      ifr.src = `googledrive://${webUrl}`;
